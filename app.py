...
#Bug fix to extend prompt charcaters beyond 50k
...
AZURE_SEARCH_ENABLE_IN_DOMAIN = "false" #os.environ.get("AZURE_SEARCH_ENABLE_IN_DOMAIN", "false")
...
AZURE_OPENAI_SYSTEM_MESSAGE=content
...
def conversation():
    ...
    logging.info(f'conversation start time: {time_before}')
    response = conversation_internal(request_body)
    time_after = datetime.now()
    logging.info(f'Conversation latency: {time_after - time_before}')
    return response
